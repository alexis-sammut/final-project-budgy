...
// Modal form for creating and editing pockets
function PocketForm({
  onClose,
  onPocketCreated,
  currency = "€",
  ...
  const [amount, setAmount] = useState(
    editingPocket?.amount
      ? parseFloat(editingPocket.amount).toFixed(2)
      : "0.00",
  );
  ...
  // Update totals when local items change
  useEffect(() => {
  ...
  // Helper to instant-save simple fields
  const autoSaveField = async (field, value) => {
    if (!editingPocket?.id) return;

    try {
      await api.patch(`/api/pockets/update/${editingPocket.id}/`, {
        [field]: value,
      });
      // Trigger parent refresh to update UI behind modal
      if (onPocketCreated) onPocketCreated();
    } catch (error) {
  ...
  const createOtherFromAmount = (pocketAmount) => {
    ...
    if (existingOther) {
      const updatedItems = localItems.map((item) =>
        item.is_other
          ? { ...item, amount: parseFloat(pocketAmount).toFixed(2), frequency }
          : item,
      );
  ...
  const createCategory = async () => {
    ...
    try {
      ...
      autoSaveField("category", res.data.id);
  ...
  // Update frequency and auto-save changes if editing
  const handleFrequencyChange = async (newFrequency) => {
    ...
    const updatedItems = localItems.map((item) => {
      if (item.is_other) {
        const convertedOtherAmount = convertAmount(
          item.amount,
          frequency,
          newFrequency,
        );
    ...
    // If editing an existing pocket, save everything immediately
    if (editingPocket?.id) {
      try {
        await api.patch(`/api/pockets/update/${editingPocket.id}/`, {
          frequency: newFrequency,
          amount: parseFloat(convertedAmount.toFixed(10)),
        });
        ...
        console.error("Error auto-saving frequency:", error);
        setError("Failed to auto-save frequency change");
      }
    }
  };
      ...
      if (editingPocket) {
        ...
        for (const item of newItems) {
          ...
          if (item.is_percentage) {
            ...
            itemData.amount = parseFloat(
              (parseFloat(item.amount) || 0).toFixed(10),
            );
        ...
        const existingItems = localItems.filter(
          (item) => !item.id.toString().startsWith("temp") && !item.is_other,
        );
                  ...
                  {colorPalette.map((paletteColor) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        autoSaveField("color", paletteColor);
                      }}
                    />
                  ))}
                </div>
              ...
              {(() => {
                ...
                return (
                  <div
                    ...
                    onMouseEnter={(e) => {
                      const icon =
                        e.currentTarget.querySelector(".info-icon-circle");
                      const tooltip =
                        e.currentTarget.querySelector(".info-text");
                      if (icon && tooltip) {
                  ...
                  >
                    ...
                    <span className="info-text">
                      This is a display based on a 30-day month. Exact amount
                      will be displayed when sorting income, based on the
                      appropriate month length.
                    </span>
                  </div>
                );
              })()}
                ...
                <input
                  ...
                  onBlur={(e) => {
                    ...
                    if (isNaN(numValue) || value === "") {
                      ...
                      const regularItems = localItems.filter(
                        (item) => !item.is_other && !item.is_percentage,
                      );
                      const itemsTotal = regularItems.reduce((sum, item) => {
                        ...
                        if (itemFreq !== frequency) {
                          return (
                            sum + convertAmount(itemAmount, itemFreq, frequency)
                          );
                        }
                        return sum + itemAmount;
                      }, 0);
                      ...
                      if (numValue < itemsTotal && regularItems.length > 0) {
                        ...
                        setError(
                          <>
                            Pocket amount cannot be less than items total
                            <br />
                            {currency}
                            {formatAmountDisplay(itemsTotal)} {freqLabel}
                          </>,
                        );
                      ...
                      if (numValue === 0) {
                        ...
                        const otherItem = localItems.find(
                          (item) => item.is_other,
                        );
            ...
            <div className="items-list">
                ...
                .map((item) => (
                  <div key={item.id} className="item-row item-recurring">
                    ...
                    <input
                      ...
                      value={
                        editingItemAmount?.[item.id] !== undefined
                  ...
                  return (
                    <div className="item-row item-other">
                      ...
                      <input
                        ...
                        className="item-amount-input"
                        value={
                          editingOtherAmount !== null
                            ? editingOtherAmount
                            : otherItem.amount_display ||
                              parseFloat(otherItem.amount)
                        }
                        onFocus={() => {
                          setEditingOtherAmount(
                            otherItem.amount_display ||
                              parseFloat(otherItem.amount),
                          );
                        }}
              ...
              <div className="add-item-section">
                ...
                <input
                  ...
                  placeholder={
                    newItemFrequency === "percentage" ? "0%" : `${currency}0.00`
                  }
                  step="0.01"
                  min="0"
        ...
        >
          <div className="confirm-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="confirm-warning">
              This only works if no pockets are using this category.
            </p>
